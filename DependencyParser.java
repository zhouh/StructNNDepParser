...
import bestFirstDepRerank.Beam;
import bestFirstDepRerank.BeamItem;
...
public class DependencyParser {
    ...
    /**
     * return the gold hierarchical state of
     * the given gold tree
     *
     * @param sent
     * @param goldTree
     * @return
     */
    public HierarchicalDepState genGoldState(CoreMap sent, DependencyTree goldTree) {

        Configuration c = system.initialConfiguration(sent);
        HierarchicalDepState pState = new HierarchicalDepState(c, -1, -1, 0.0, 0.0, null, true);

        while (!system.isTerminal(pState.c)) {

            Pair<Integer, Integer> oraclePair = system.getHierarchicalOracle(pState.c, goldTree);
...
//                optActType = goldTrans.first;
    ...
    /**
     * get the revised item for a gold tree
     *
     * @param sentence
     * @param goldTree
     * @return
     */
    public List<ReviseItem> getRevisedItem(CoreMap sentence, DependencyTree goldTree) {
        int currentDepth = 0;
        int oracleDepth = config.nOracleDepth;

        int index = 0;

        List<ReviseItem> retval = new ArrayList<>();
...
//                optActType = goldTrans.first;
                ...
                retval.add(new ReviseItem(index, goldTrans.first, -1));
            }
    ...
    public void outputWrongActProb(String testFile, String outFile) throws IOException {
        ...
        double totalReviseNum = 0;

        for (int i = 0; i < testSents.size(); i++) {
            totalReviseNum += wrongActAnalysis(testSents.get(i), testTrees.get(i), output);
        }


        System.err.println("###### totally "+ (totalReviseNum / testSents.size()) + " number of error per sentence.");
        output.close();
        timer.done();
     ...
     */
    private int wrongActAnalysis(CoreMap sentence, DependencyTree goldTree, PrintWriter output) {
...
//		  Pair<Integer, Integer> optActPair = classifier.computeHierarchicalScore(feature, c);
     ...
     */
    private List<Pair<Double, DependencyTree>> bestfirstRevise(CoreMap sent, DependencyTree goldTree) {
	 ...
	 */
        ...
        Pair<Double, Double> originUASPair = system.evaluateOneTree(sent, initState.c.tree, goldTree);
...
//                Pair<Double, Double> UASPair = system.evaluateOneTree(sent, state.c.tree, goldTree);


//                double initStateScore = -1.0 / UASPair.first;  ///////////////////////////////////////////////
...
//                            System.out.println("margin = " + margin);
                        ...
                        if (margin < config.dMargin &&
                                label[j] == 0){
                            RevisedState revisedState = new RevisedState(new ReviseItem(-1, j, margin), state, initStateScore);
                            revisedState.setInitScore(originUASPair.first);
                            revisedItemsFromOneState.add(revisedState);
                        }
...
//                if(UASPair.first < rs.initScore)
//                    continue;
//                double initStateScore = -1.0 / UASPair.first;  ///////////////////////////////////////////////

//                double initStateScore = -1;  ///////////////////////////////////////////////
                ...
                while (state.lastState != rs.state) { //till the previous revised state
                    ...
                    for (int j = 0; j < ParsingSystem.nActTypeNum; j++) {
                        ...
                        if (margin < config.dMargin){
                            RevisedState revisedState = new RevisedState(new ReviseItem(-1, j, margin), state, initStateScore);
                            revisedState.setInitScore(UASPair.first);
                            revisedItemsFromOneState.add(revisedState);
    ...
    private List<Pair<Double, DependencyTree>> beamSearchDecode(CoreMap sent, DependencyTree goldTree) {
        ...
        retval.add(Pair.makePair(initBeamItem.score, initState.c.tree));  // add to kbest list
            ...
            while (beam.size() != 0) {
                ...
                while (currentState.lastState != revisedPoint) {
                    ...
                    for (int j = 0; j < ParsingSystem.nActTypeNum; j++) {
                        ...
                        if (margin < config.dMargin &&
                                label[j] == 0)
                            revisedItemsFromOneState.add(new RevisedState(new ReviseItem(-1, j, margin), currentState, -1));
                ...
                for (int k = 0; k < config.nMaxReviseActNum; k++) {
                    ...
                    HierarchicalDepState state =
                            partialGreedyParser(revisedState.state, revisedState.item.reviseActID, sent, true);
                    BeamItem currentBeamItem = new BeamItem(state, revisedState.state, state.score / actSize);
                    nextBeam.insert(currentBeamItem);
                    retval.add(Pair.makePair(currentBeamItem.score, state.c.tree));  // add to kbest list
    ...
    private void getNBest(String testFile, String outputFile) throws IOException {
        ...
        for (int j = 0; j < testSents.size(); j++) {
            ...
            DependencyTree goldTree = goldTrees.get(j);

            if (config.bBeamNBest) {
                outputNBest.add(beamSearchSample(sent));
            } else if (config.bBestFirstRevise)
                outputNBest.add(bestfirstRevise(sent, goldTree));
            else if(config.bUseBeamSample)
                outputNBest.add(beamSearchDecode(sent, goldTree));
...
//			output.println(nbest.get(k).second);
                ...
                //get oracle
                double f1 = system.evaluateOneTree(testSents.get(j), nbest.get(k).second, goldTrees.get(j)).first;
                ...
                output.println(nbest.get(k).first + "\t" + nbest.get(k).second.toBracketString(testSents.get(j)));
            }
